...
pub async fn handle_request(state: &SsoState, target: &str, payload: &Value) -> Response {
    let action = target.strip_prefix("SWBExternalService.").unwrap_or(target);
...
fn delete_permission_set(state: &SsoState, body: &Value) -> Result<Response, LawsError> {
    ...
    state
        .permission_sets
        .remove(permission_set_arn)
        .ok_or_else(|| {
            LawsError::NotFound(format!("permission set not found: {permission_set_arn}"))
        })?;
